...
object Hello extends JFXApp {
  ...
  val mainScene: Scene = new Scene{
    fill = Green
    root = MainPane(startSimulation,startGame,helpInfo)
  }

  def startMenu(): Unit = {
    if(game!=null) game.status = GameStatus.Ended
    if(player!=null) player.isSelected = true
    mainScene.root = MainPane(startSimulation,startGame,helpInfo)
  }
  ...
  def helpInfo() ={
    val helpPane = HelpPane(startMenu)
    mainScene.root = helpPane
  }
